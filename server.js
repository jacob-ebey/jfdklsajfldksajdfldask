...
function addLoggingToDataFunc(id, type, func, config) {
  return async (...args) => {
    try {
      return await func(args);
    } catch (error) {
      console.error(
        `Error in ${type} function for route ${id}:`,
        config,
        error
      );
    }
...
async function reimportServer() {
  ...
  const build = import(BUILD_URL + "?t=" + stat.mtimeMs);

  return addLoggingToBuild(build);
}
